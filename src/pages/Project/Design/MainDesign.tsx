...
import { DesignData } from "../../utils/contentData";
...
const MainDesign = () => {
  const [currentPage, setCurrentPage] = useState(1);
    ...
    const totalPages = Math.ceil(DesignData.length / ITEMS_PER_PAGE);
  
    // Calculate paginated items
    const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
    const showItems = DesignData.slice(startIndex, startIndex + ITEMS_PER_PAGE);
    ...
    const handleCardClick = (item: DesignItem) => {
  ...
  return (
    <div className="flex flex-col gap-6 lg:flex-row justify-between bg-[--text-extra] px-6 sm:px-12 lg:px-20 py-10">
      ...
      <div className="w-full bg-[--text-extra] lg:grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-1 gap-6 lg:mx-5">
        <h2 className="text-2xl font-bold border-b-[4px] border-[--bg-color] pb-3">Latest Design Project</h2>
